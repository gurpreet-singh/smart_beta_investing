...
Applies the same analysis to:
  ...
  - Nifty 200 Value 30 Index

Output: nifty200/output/nifty200_returns_analysis.json
"""
...
MOMENTUM_CSV = BASE_DIR / "output" / "monthly" / "nifty200_momentum_30_monthly.csv"
VALUE_CSV    = BASE_DIR / "output" / "monthly" / "nifty200_value_30_monthly.csv"
OUTPUT_DIR   = BASE_DIR / "output"
OUTPUT_FILE  = OUTPUT_DIR / "nifty200_returns_analysis.json"
...
def compute_cagr(start_value: float, end_value: float, years: float) -> float | None:
    """
    Returns annualised CAGR as a percentage.
...
def compute_rolling_cagrs(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns a DataFrame indexed by Date with columns: 1Y_CAGR, 3Y_CAGR, 5Y_CAGR, 10Y_CAGR.
    """
    results = []
    ...
    for i, row in df.iterrows():
        ...
        for label, years in PERIODS.items():
            # Need the row approximately N years ago
            target_date = end_date - pd.DateOffset(years=years)
            # Find the nearest month-end at or before target_date
            ...
            if len(past_rows) == 0:
                ...
                # Actual elapsed years (in fractional years using days)
...
def main():
    print("=" * 60)
    print("  Nifty 200 Returns Analysis")
    print("=" * 60)

    # Momentum
    print("\n📊 Loading Nifty 200 Momentum 30 data …")
    mom_df = load_monthly_data(MOMENTUM_CSV)
    print(f"   Rows loaded: {len(mom_df)}  |  Range: {mom_df['Date'].min().date()} → {mom_df['Date'].max().date()}")

    mom_cagr = compute_rolling_cagrs(mom_df)
    print(f"   1Y CAGR rows available : {mom_cagr['CAGR_1Y'].notna().sum()}")
    print(f"   3Y CAGR rows available : {mom_cagr['CAGR_3Y'].notna().sum()}")
    print(f"   5Y CAGR rows available : {mom_cagr['CAGR_5Y'].notna().sum()}")
    print(f"   10Y CAGR rows available: {mom_cagr['CAGR_10Y'].notna().sum()}")

    # Value
    print("\n📊 Loading Nifty 200 Value 30 data …")
    val_df = load_monthly_data(VALUE_CSV)
    print(f"   Rows loaded: {len(val_df)}  |  Range: {val_df['Date'].min().date()} → {val_df['Date'].max().date()}")

    val_cagr = compute_rolling_cagrs(val_df)
    print(f"   1Y CAGR rows available : {val_cagr['CAGR_1Y'].notna().sum()}")
    print(f"   3Y CAGR rows available : {val_cagr['CAGR_3Y'].notna().sum()}")
    print(f"   5Y CAGR rows available : {val_cagr['CAGR_5Y'].notna().sum()}")
    print(f"   10Y CAGR rows available: {val_cagr['CAGR_10Y'].notna().sum()}")
    ...
    def df_to_chart_series(cagr_df: pd.DataFrame) -> dict:
        """Convert rolling CAGR DataFrame to chart-ready dict series."""
        series = {}
    ...
    print(f"\n✅ JSON saved: {OUTPUT_FILE}  ({file_size_kb:.1f} KB)")
    print("\n" + "=" * 60)
